...
def train_ml_models(X_scaled, y, metadata):
    ...
    # Hyperparameter grids (ALL VALUES AS LISTS - FIX FOR NESTED CV)
    param_grids = {
        'Random Forest': {
        ...
        'Gradient Boosting': {
            'n_estimators': [100, 200],
            'learning_rate': [0.05, 0.1],  # FIXED: Now a list
            'max_depth': [3, 5]
        },
        'Ridge': {'alpha': [0.1, 1.0, 10.0]},
        'Lasso': {'alpha': [0.001, 0.01, 0.1]},
        'ElasticNet': {'alpha': [0.1, 1.0], 'l1_ratio': [0.3, 0.5, 0.7]},
        'SVR': {'C': [1, 10], 'kernel': ['rbf', 'linear'], 'gamma': ['scale', 'auto']}
    }
