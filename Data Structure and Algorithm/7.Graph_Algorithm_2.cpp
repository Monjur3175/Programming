...
struct AStarNode {
    int x, y, g, h;
    bool operator<(const AStarNode& other) const { // for priority queue, smallest f=g+h on top
...
bool hasCycle(int V, vector<pair<int,int>>& edges) {
    UnionFind uf(V);
    for(auto& e : edges) {
        if(!uf.unite(e.first, e.second)) return true;
    }
    return false;
}
...
void dfsForSCC(int u, vector<vector<int>>& adj, vector<bool>& visited, vector<int>& order) {
    visited[u] = true;
    for(int v : adj[u]) if(!visited[v]) dfsForSCC(v, adj, visited, order);
    order.push_back(u);
}
...
void dfsOnTranspose(int u, vector<vector<int>>& transpose, vector<bool>& visited) {
    visited[u] = true;
    cout << u << " ";
    for(int v : transpose[u]) if(!visited[v]) dfsOnTranspose(v, transpose, visited);
}
...
void kosarajuSCC(vector<vector<int>>& adj) {
    ...
    for(int i=0; i<V; i++) if(!visited[i]) dfsForSCC(i, adj, visited, order);
...
int main() {
    ...
    if(topoOrder.empty()) cout << "Cycle detected in graph - no topo sort\n";
    else {
